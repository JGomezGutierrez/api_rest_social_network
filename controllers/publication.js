...
//Metodo para hacer una publicacion
export const savePublication = async (req, res) => {
    try {
      ...
      //Verificar que llegue desde el body el parametro text con su infomacion
      if (!params.text){
      ...
      //Agregar la informacion del usuario autenticado al objeto de la nueva publicacion 
      newPublication.user_id = req.user.userId;

      //Guardar la nueva publicacion
      const publicationStored = await newPublication.save();

      //Verificar si se guardo la publicacion en la base de datos 
...
export const feed = async (req, res) => {
  try {
    ...
    if (!myFollows.user_following || myFollows.user_following.length === 0){
      return res.status(404).send({
        status: "error",
    ...
    const result = await Publication.paginate(
      { user_id: { $in: myFollows.user_following }},
      options
    );
